...
function randomItems.onUse(player, item, fromPosition, target, toPosition, isHotkey)
    ...
    local discordMessage = ":package: **" .. player:getName() .. "** opened a _" .. item:getName() .. "_ and received **" .. rewardItem.name .. "**."
    Webhook.sendMessage(discordMessage, announcementChannels["log-bags"]) -- Certifique-se de configurar "log-bags".

    -- Opcional: Mensagem para o servidor (broadcast para todos)
    -- Game.broadcastMessage(player:getName() .. " opened a " .. item:getName() .. " and received " .. rewardItem.name .. ".", MESSAGE_EVENT_ADVANCE)
